...
export function SignInButton({ label = 'Continue with Google' }: { label?: string }) {
  return (
    <button
      ...
      className="group flex w-full items-center justify-center gap-3 rounded-2xl bg-white px-5 py-4 text-lg font-semibold text-slate-800 shadow-lg transition active:scale-[0.99]"
    >
      <GoogleMark />
      {label}
    </button>
